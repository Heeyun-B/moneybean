...
class DepositProducts(models.Model):
    ...
    join_deny = models.IntegerField(choices=[(1, '제한없음'), (2, '서민전용'), (3, '일부제한')], default=1)  # 가입제한
    ...
    max_limit = models.BigIntegerField(blank=True, null=True)  # 최고한도 (금액이 클 수 있음)
    ...
    fin_co_subm_day = models.CharField(max_length=12, blank=True, null=True)  # 금융회사 제출일
...
class DepositOptions(models.Model):
    """정기예금 상품 옵션 정보"""
    product = models.ForeignKey(DepositProducts, on_delete=models.CASCADE, related_name='options', to_field='fin_prdt_cd')  # 상품 외래키
    intr_rate_type = models.CharField(max_length=10)  # 저축 금리 유형 (S: 단리, M: 복리)
