<template>
  <main class="main-content" v-if="savingDetail">
    <div class="detail-header">
      <button @click="goBack" class="back-btn">← 뒤로가기</button>
      <p class="bank-name">{{ savingDetail.product.kor_co_nm }}</p>
      <h2 class="product-title">{{ savingDetail.product.fin_prdt_nm }}</h2>
    </div>
      ...
      <section class="info-section">
        <div class="info-card">
          <div class="info-item">
            ...
            <p>{{ savingDetail.product.join_member }}</p>
          </div>
          <div class="info-item">
            <span class="label">가입 방법</span>
            <p>{{ savingDetail.product.join_way }}</p>
          </div>
          <div class="info-item">
            <span class="label">우대 조건</span>
            <p class="text-block">{{ savingDetail.product.spcl_cnd || '해당 사항 없음' }}</p>
          </div>
          <div class="info-item">
            <span class="label">만기 후 이자율</span>
            <p class="text-block">{{ savingDetail.product.mtrt_int }}</p>
          </div>
        </div>
      </section>
      ...
      <section class="rate-section">
        ...
        <div class="table-container">
          <table class="rate-table">
            ...
            <tbody>
              <tr v-for="option in savingDetail.options" :key="option.id">
...
const savingDetail = computed(() => store.savingDetail);
const isSubscribed = ref(false);

// 뒤로가기 - 탭 유지
const goBack = () => {
  const fromTab = route.query.from || 'saving';
...
const checkSubscriptionStatus = async () => {
  if (authStore.token) {
    await store.getMySavingSubscriptions(authStore.token);
    const prdtCd = route.params.id;
    
    isSubscribed.value = store.mySavingSubscriptions.some((sub) => {
      return String(sub.product.fin_prdt_cd || sub.product).trim() === String(prdtCd).trim();
    });
  }
};
...
onMounted(async () => {
  const prdtCd = route.params.id;
  await store.getSavingDetail(prdtCd);
...
const handleSubscribe = () => {
  ...
  const firstOptionId = savingDetail.value.options[0]?.id;

  store.subscribeSaving(finPrdtCd, firstOptionId)
    .then(() => {
...
const handleUnsubscribe = () => {
  ...
  store.unsubscribeSaving(finPrdtCd)
    .then(() => {
...
.subscribe-btn, .unsubscribe-btn { border: none; padding: 15px 60px; border-radius: 30px; font-size: 18px; font-weight: 600; cursor: pointer; transition: background 0.2s; color: white; }
